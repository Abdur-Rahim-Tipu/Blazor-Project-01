...
namespace CandidateBlazorApp.Server.Controllers
{
    ...
    public class CandidatesController : ControllerBase
    {
        ...
        [HttpPost("File")]
        public async Task<FileUploadResponse> UploadFill(IFormFile file)
        {
            var ext = Path.GetExtension(file.FileName);
            var randomFileName = Path.GetFileNameWithoutExtension(Path.GetRandomFileName());
            var storedFileName = randomFileName + ext;
            using FileStream fs = new FileStream(Path.Combine(env.WebRootPath, "UploadFile", storedFileName), FileMode.Create);
            await file.CopyToAsync(fs);
            fs.Close();
            return new FileUploadResponse { UploadFile = file.FileName, UploadFileName = storedFileName };
        }
    }
}
